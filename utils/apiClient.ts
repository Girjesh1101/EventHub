...
import { reportStep } from "./allureReports";
...
export class ApiClient {
    ...
    async get(endpoint: string){
        ...
        return await reportStep(`API GET ${endpoint}`, async ()=>{
    ...
    async post(endpoint:string, payload?: unknown){
        const url = `${envConfig.apiBaseURL}${endpoint}`;
        return await reportStep(`API POST ${endpoint}`, async ()=>{
    ...
    async delete(endpoint:string){
        const url = `${envConfig.apiBaseURL}${endpoint}`;
        return await reportStep(`API DELETE ${endpoint}`, async ()=>{
    ...
    async put(endpoint:string, payload?: unknown){
        const url = `${envConfig.apiBaseURL}${endpoint}`;
        return await reportStep(`API PUT ${endpoint}`, async ()=>{
